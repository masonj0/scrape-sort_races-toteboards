...
def _convert_to_normalized_race(model_race: Race, prog_num_map: Dict[Tuple[str, str], int]) -> NormalizedRace:
    ...
    runners = [
        NormalizedRunner(
            name=r.name,
            odds=r.odds,
            program_number=prog_num_map.get((model_race.race_id, r.name), i + 1)
...
async def run_pipeline(
...
) -> Tuple[List[NormalizedRace], Dict[str, int]]:
    """
    Orchestrates the end-to-end pipeline.
    Returns a tuple of (races, funnel_statistics).
    """
    ...
    funnel_stats['enabled_adapters'] = len(adapter_classes)
    ...
    for adapter_class in adapter_classes:
        ...
        try:
            ...
            logging.error(f"An error occurred in the '{source_id}' adapter.", exc_info=True)
        finally:
            if ui:
    ...
    funnel_stats['successful_adapters'] = sum(1 for count in races_per_adapter.values() if count > 0)
    funnel_stats['initial_races'] = len(unmerged_races)
    ...
    funnel_stats['merged_races'] = len(merged_model_races)

    scorer = RaceScorer()
    for race in merged_model_races:
        setattr(race, 'score', scorer.score(race))

    races_before_runner_filter = len(merged_model_races)
    if min_runners > 1:
        merged_model_races = [r for r in merged_model_races if r.number_of_runners >= min_runners]
    funnel_stats['after_runner_filter'] = len(merged_model_races)
    ...
    funnel_stats['after_time_filter'] = len(final_normalized_races)

    final_normalized_races.sort(key=lambda r: r.score or 0, reverse=True)
    funnel_stats['final_races'] = len(final_normalized_races)

    logging.info("--- Pipeline End ---")
    return final_normalized_races, funnel_stats
